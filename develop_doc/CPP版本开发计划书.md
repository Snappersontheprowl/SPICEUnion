...
- 解析路径：libpsf 是 Python 包装 + 解析器；C++ 自写 PSF 解析器，单文件解析吞吐通常可提升一个数量级。这是"底层服务"最实打实的价值。
...
"多仿真器抽象 + 原生结果解析 + 底层调度"是一组可以讲深讲透、可量化验证的工程故事，比 Python 胶水层有更强的技术纵深。
...
- **V1（必做，核心）**：完整覆盖当前 Python 执行层功能 + C++ 核心 + Spectre 单仿真器 + 进程池/线程池 + 原生 PSF 解析 + C ABI + Python 绑定 + 性能基准。V1 是总目标，实际开发按 3.5 的启动切片推进。
