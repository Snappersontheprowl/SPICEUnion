...
void expect_rc_lowpass_ac_semantics(const su::AcResponse& response, double resistance_ohm,
...
double interpolate_at_time(const su::TranWaveform& waveform, double target_time_s) {
...
void expect_rc_charging_tran_semantics(const su::TranWaveform& waveform, double resistance_ohm,
                                       double capacitance_f, double input_voltage_v) {
...
TEST(NgspiceSessionExternalTest, RunsRcLowPassAcWhenExternalNgspiceIsEnabled) {
  ...
  expect_rc_lowpass_ac_semantics(ac.value, 1000.0, 1.0e-12);
...
TEST(NgspiceSessionExternalTest, RunsRcChargingTranWhenExternalNgspiceIsEnabled) {
  ...
  expect_rc_charging_tran_semantics(waveform.value, 1000.0, 1.0e-12, 1.0);
