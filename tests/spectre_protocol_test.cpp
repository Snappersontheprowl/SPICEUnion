...
TEST(SpectreProtocolTest, FormatsParameterUpdateAndRunCommand) {
  ...
  EXPECT_NE(command.find("(sclSetAttribute (sclGetParameter top \"wp1\") \"value\" 1.250000e-06)"),
            std::string::npos);
  EXPECT_NE(command.find("(sclSetAttribute (sclGetParameter top \"wn1\") \"value\" 2.000000e-06)"),
            std::string::npos);
...
TEST(SpectreProtocolTest, RecognizesResourceStatsThenTAsSuccess) {
  ...
  EXPECT_EQ(su::classify_spectre_completion_line("Peak resident memory used = 1 Mbytes", &seen),
            su::SpectreCompletion::kContinue);
  EXPECT_TRUE(seen);
  EXPECT_EQ(su::classify_spectre_completion_line("t", &seen), su::SpectreCompletion::kSucceeded);
}

TEST(SpectreProtocolTest, RecognizesNilAfterResourceStatsAsFailure) {
  bool seen = true;

  EXPECT_EQ(su::classify_spectre_completion_line("nil", &seen), su::SpectreCompletion::kFailed);
}
