...
| `spectre_materials_dc_op.raw/` | `dcOp.dc` | 真实 Spectre 23.1 运行产物（私有材料，原始路径不公开） | 当前 Cadence Spectre 23.1 输出的 DC scalar 读取 | `net6 = 0.8` |
| `amp_ac_response.raw/` | `ac.ac` | 真实 Spectre 23.1 运行产物（私有材料，原始路径不公开） | 标准 AC swept complex response 读取 | `freq[0] = 1`，`freq[1]≈1.51356`，`net1[0]≈(1.00000109476, -3.33776e-7)`，共 51 点 |
...
| `spectre_materials_stb_loop_gain.raw/` | `stb.stb` | 真实 Spectre 23.1 运行产物（私有材料，原始路径不公开） | STB / 频域 swept complex response 读取 | `freq[0] = 1`，`freq[1]≈1.12202`，`loopGain[0]≈(-287890, 26932.1)`，共 201 点 |
...
| `spectre_materials_psfxl_tran.raw/` | `tran.tran.tran` + `.psfxl` + `.sig` | 真实 Spectre 23.1 运行产物（私有材料，原始路径不公开） | Spectre 23.1 PSFXL transient 边界样本 | 当前 `henjo/libpsf` backend 明确返回 `kUnsupportedFormat` |
