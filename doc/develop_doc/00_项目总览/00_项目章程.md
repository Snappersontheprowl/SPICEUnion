...
| M5 | C++ workflow facade 已完成 | `Simulation` / `SimulationResult` 用户工作流入口；Python / MATLAB 入口暂缓 |
| M6 | 暂缓 | 性能基准与发布形态 |
...
1. Python workflow binding：根据真实 Python 调用需求绑定 `Simulation` / `SimulationResult`。
2. M6：在有稳定 baseline 后建立可复现 benchmark，不把未实测性能数字写成完成事实。
