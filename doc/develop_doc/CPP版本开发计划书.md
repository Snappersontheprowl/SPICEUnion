...
| M3 多仿真器抽象（1–2 周） | 先用 Ngspice 最小 batch adapter 验证第二真实仿真器，再用 RC AC / RC TRAN 这类真实小任务收敛结果 IR；Xyce/Hspice 不提前占位 | Ngspice AC / TRAN 能映射到已有 `AcResponse` / `TranWaveform`；至少两个仿真器能输出同类 IR；IR 扩展有真实消费者；文档记录保留与拒绝的抽象 |
